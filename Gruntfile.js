...
module.exports = function(grunt) {
		...
		.initConfig({
			...
			copy: {
				main: {
					expand: true,
					cwd: '.',
					src: ['*.php', 'img/**/*', 'public/**/*', 'includes/**/*', 'languages/**/*', 'admin/**/*', 'assets/**/*', 'doc/**/*'],
			...
			watch: {
				...
				gui: {
					files: ['public/assets/css/src/**/*.less', 'includes/*.html', 'public/assets/js/dist/*.js', '**/*.php'],
					tasks: ['deploy'],
					options: {
						spawn: false,
					}
			...
			//			},
			//			src : '.',
			//			dest : '/www.fornitoreoffresi.com/wp/wp-content/plugins/standselector',
			//			exclusions : [ 'node_modules', '.phpintel', '.grunt', '.DS_Store', '.ftppass', '.git*',
			//				'ftpcache.json' ],
			//			simple : true,
	...
	grunt.registerTask('deploy', ['build', 'bump-only', 'regex-replace', 'bump-commit']);
